function [output] = dpca_analysis_3d(firingRates, firingRatesAverage, trialNum, margNames)
...
combinedParams = {{1, [1 4]},{2, [2 4]}, {3, [3 4]}, {4},  ...
                  {[1 2], [1 2 4], [1 3], [1 3 4], [2 3], [2 3 4], [1 2 3], [1 2 3 4]}};
margColours = [23 100 171; 187 20 25; 30 180 30; 150 150 150; 114 97 171]/256;
...
for n = 1:size(firingRates,1)
    for s = 1:size(firingRates,2)
        for d = 1:size(firingRates,3)
            for k = 1:size(firingRates,4)
                assert(isempty(find(isnan(firingRates(n,s,d,k,:,1:trialNum(n,s,d,k))), 1)), 'Something is wrong!')
            end
        end
    end
end
